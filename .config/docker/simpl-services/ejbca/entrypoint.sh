...
export HTTPSERVER_HOSTNAME="simpl-control-plane"
export MANAGEMENT_END_ENTITY_USERNAME="SuperAdmin"
export MANAGEMENT_END_ENTITY_PASSWORD="123-stella"
export DATABASE_JDBC_URL="jdbc:postgresql://local-authority-postgresql.local-authority.svc.cluster.local:5432/authority_ejbca"
export DATABASE_USER="ejbca"
export DATABASE_PASSWORD="ejbca"
export T1_GATEWAY="http://tier1-gateway-authority:8100"
# export NAMESPACE="local-authority"
...
function do_it () {
  ...
  cp -Rfv /tmp/config/* /opt/keyfactor/import/profiles
  ...
  [[ "$T1_GATEWAY" =~ ^https?:// ]] || T1_GATEWAY="https://$T1_GATEWAY"

  "${EJBCA_SH}" ca editca OnBoardingCA defaultCRLDistPoint "${T1_GATEWAY}/crl/OnBoardingCA"
  "${EJBCA_SH}" ca editca OnBoardingCA defaultOCSPServiceLocator "${T1_GATEWAY}/ocsp"
  "${EJBCA_SH}" ca editca OnBoardingCA certificateAiaDefaultCaIssuerUri "${T1_GATEWAY}/ca/OnBoardingCA"

  #################
  #   MNGT USER   #
  #################
  # if ! kubectl -n "${NAMESPACE}" get secret ejbca-rest-api-secret >/dev/null 2>&1; then
    echo "####################################################"
    echo "Secret containing creating it..."
    echo "####################################################"
    ...
    # kubectl -n "${NAMESPACE}" create secret generic ejbca-rest-api-secret \
