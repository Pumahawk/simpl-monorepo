...
function do_it () {
  cat >&2 <<'EOF'
====================================================
    New Entrypoint! Adapted for Docker Compose!
====================================================
EOF
  ...
  tail -f /opt/keyfactor/starting-ejbca.log &
  local pid_tail_ejbca_logs=$!
  ...
  #   MNGT USER   #
  #################
  if [ ! -f "/output/secret/client-cert.p12" ] || [ ! -f "/output/secret/ca-truststore.jks" ]; then
    echo "####################################################"
    echo "Secret not found in /output/secret, creating it..."
    echo "####################################################"
    ...
    "${EJBCA_SH}" batch "${MANAGEMENT_END_ENTITY_USERNAME}"

    curl -Lk "https://localhost:8443/ejbca/ra/cert?caid=$("${EJBCA_SH}" ca info ManagementCA | grep "CA ID" | sed 's/.*CA ID: //')&chain=true&format=jks" \
      > /opt/keyfactor/p12/truststore.jks

    # Copia dei file nel volume condiviso
    mkdir -p /output/secret
    cp "/opt/keyfactor/p12/${MANAGEMENT_END_ENTITY_USERNAME}.p12" /output/secret/client-cert.p12
    cp "/opt/keyfactor/p12/truststore.jks" /output/secret/ca-truststore.jks

    echo "Secret material saved in /output/secret"
  else
    echo "Secret files already exist in /output/secret"
  fi
}
